...
theta_all = read.csv("./Scenarios_parameters/theta_all_final_high.csv")
...
for(i in 1:length(theta_all[,1])){
 ...
 #write.table(output, "~/Dropbox/RGNOSIS/Methods/Empiric prescribing model/Model/Output/Final/Sens/Output_empiric_base_antib_high.csv", append=T, sep = ",",row.names=T,col.names=ifelse(i==1,NA,F))
 #write.csv(traj, paste0("~/Documents/Esther/RGNOSIS/Methods/Empiric_prescribing_model/Output/Sensitivity analysis/Trajectories/Traj_empiric_base_antib_high_",i,".csv"))
}